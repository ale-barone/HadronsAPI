...
// check if string "toFind" is in "string"
bool has_string(std::string string, std::string toFind){
    size_t pos = string.find(toFind);
    bool has = (pos != std::string::npos) ? true : false;
    return has;
}
...
size_t pos_5th_underscore(std::string string, std::string begin){
	std::string substr = copy_from(string, begin);
    size_t pos = substr.size();
	char underscore = '_';
	int num_underscore = 0;
	for (int i=0; i<substr.size(); i++){
		if (substr[i] == underscore){
			num_underscore++;
        }
		if (num_underscore == 5){
			pos = i;
...
size_t pos_4th_underscore(std::string string, std::string begin){
	std::string substr = copy_from(string, begin);
    size_t pos = substr.size();
	char underscore = '_';
	int num_underscore = 0;
...
size_t pos_6th_underscore(std::string string, std::string begin){
	std::string substr = copy_from(string, begin);
    size_t pos = substr.size();
	char underscore = '_';
	int num_underscore = 0;
...
// get momentum in a string with the "snkmom_" keyword
std::string get_snkmom(std::string string){
    std::string mom_full = copy_from(string, "snkmom_");
	std::string mom_string = copy_from(mom_full, "snkmom_", pos_5th_underscore(mom_full, "snkmom_"));
	std::string mom = remove_str(mom_string, "snkmom_");
	return mom;
}
...
std::string get_mom(std::string string){
	...
	std::string mom_string = copy_from(mom_full, "mom_", pos_5th_underscore(mom_full, "mom_"));
...
std::string get_twist(std::string string){
	...
	std::string tw_string = copy_from(tw_string_full, "tw_", pos_5th_underscore(tw_string_full, "tw_"));
...
std::string remove_twist_str(std::string string){ 
    std::string toErase = copy_from(string, "_tw_", pos_6th_underscore(string, "_tw_"));
    size_t pos = string.find(toErase);
    if (pos != std::string::npos){   
        string.erase(pos, toErase.length());
    }
