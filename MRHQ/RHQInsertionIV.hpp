...
void TRHQInsertionIV<FImpl, GImpl>::execute(void)
{
    ...
    auto gauge_x = peekLorentz(gaugefield, 0);
    auto gauge_y = peekLorentz(gaugefield, 1);
    auto gauge_z = peekLorentz(gaugefield, 2);

    ////////////////////////////////////////////////////////////////////////////
    // In case the propagator we are applying the RHQInseriton on contains twist
    // we attach the twist to the gauge field as in 
    ...
    if (!par().propTwist.empty())
    {
        ...
        ComplexD i(0.0,1.0);
        ComplexD ph_x = exp(i*q_x);
        ComplexD ph_y = exp(i*q_y);
        ComplexD ph_z = exp(i*q_z);

        gauge_x = ph_x*gauge_x;
        gauge_y = ph_y*gauge_y;
        gauge_z = ph_z*gauge_z;
    }
    ////////////////////////////////////////////////////////////////////////////
